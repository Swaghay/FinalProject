(* These are the functions and modules I get from the module in the lib directory.
   I've called it FinalProject.ml there, some will probably have called it halloween.ml.
...
let string_of_declaration = FinalProject.string_of_declaration
let mainParser = FinalProject.Parser.main
let mainLexer = FinalProject.Lexer.token
module Parser : (sig exception Error end) = FinalProject.Parser
(* Here are the types these things have for me,
   ...
   Parser.main : (Lexing.lexbuf -> token) -> Lexing.lexbuf -> FinalProject.declaration list
   Lexer.token : Lexing.lexbuf -> FinalProject.Parser.token
   string_of_declaration : FinalProject.declaration -> string
*)
